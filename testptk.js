// ==UserScript==
// @id             iitc-plugin-player-tracker-lite@kik0220
// @name           IITC Plugin: Player tracker Lite
// @category       Layer
// @version        0.0.1.20150627.4522
// @namespace      https://github.com/kik0220
// @updateURL      https://raw.githubusercontent.com/kik0220/public/master/testptk.js
// @downloadURL    https://raw.githubusercontent.com/kik0220/public/master/testptk.js
// @description    [jonatkins-2015-06-27-004522] Draw trails for the path a user took onto the map based on status messages in COMMs. Uses up to three hours of data. Does not request chat data on its own, even if that would be useful.
...
plugin_info.pluginId = 'player-tracker-lite';
...
window.plugin.playerTracker.setup = function() {
  ...
  $('<style>').prop('type', 'text/css').html('.plugin-player-tracker-popup-label { font-family: Arial, sans-serif; opacity: 0.8; background-color: floralwhite;}').appendTo('head');
...
window.plugin.playerTracker.drawData = function() {
  ...
  $.each(plugin.playerTracker.stored, function(plrname, playerData) {
    ...
    var m = L.marker(gllfe(last), {icon: icon, referenceToPortal: closestPortal, opacity: absOpacity, desc: popup[0], title: tooltip, clickable: false, keyboard: false});
    var l = playerData.nick.length * 8;
    var m2 = L.marker(gllfe(last), {icon: L.divIcon({
        className: 'plugin-player-tracker-popup-label',
        iconAnchor: [l/2,60],
        iconSize: [l,22],
        html: playerData.nick})
    , clickable: false, keyboard: false});
    ...
    m2.addTo(playerData.team === 'RESISTANCE' ? plugin.playerTracker.drawnTracesRes : plugin.playerTracker.drawnTracesEnl);
    window.registerMarkerForOMS(m);
    window.registerMarkerForOMS(m2);
